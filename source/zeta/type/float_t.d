...
class ZtFloatType : ZtType {
    ...
    override @property string name() { return "float"; }
    
    override @property string op_tostring(ZtValue* self) {
        return self.m_float.text;
    }

    override bool op_eval(ZtValue* self) { return self.m_float > 0; }

    override ZtValue op_cast(ZtValue* self, ZtType type) {
        if (type == this) return self.deRefed;

        else if (type == interpreter.stringType) return interpreter.stringType.make(self.m_float.text);
        else if (type == interpreter.integerType) return interpreter.floatType.make(cast(int)self.m_float);
        else return super.op_cast(self, type);
    }

    override bool op_equal(ZtValue* self, ZtValue rhs) {
        if (rhs.type == this) return self.m_float == rhs.m_float;
        else if (rhs.type == interpreter.integerType) return self.m_float == rhs.m_int;
    ...
    override int op_cmp(ZtValue* self, ZtValue rhs) {
        float result;
        if (rhs.type == this) result = self.m_float - rhs.m_float;
        else if (rhs.type == interpreter.integerType) result = self.m_float - rhs.m_int;
        else return super.op_cmp(self, rhs);
    ...
    override ZtValue op_add(ZtValue* self, ZtValue rhs) {
        if (rhs.type == this) return make(self.m_float + rhs.m_float);
        else if (rhs.type == interpreter.integerType) return make(self.m_float + rhs.m_int);
