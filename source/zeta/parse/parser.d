...
struct ZtParser {
    ...
    private {
	    ...
	    ZtAstDef parseDefParameter() {
		    if (this.testForToken(ZtToken.Type.ud_attribute)) this.parseAttributes();
	    ...
	    ZtAstFunction parseFunction() {
		    ...
		    if (this.advanceForToken(ZtToken.Type.tk_semicolon)) node.isLinkage = true;
	    ...
	    ZtAstIf parseIf() {
		    ...
		    if (this.advanceForToken(ZtToken.Type.kw_else)) node.elseMembers = this.parseBlock!parseStatement();
		    return node;
		}
	    ...
	    ZtAstSwitch parseSwitch() {
		    ...
		    node.subject = this.parseExpression();
		    this.expectToken(ZtToken.Type.tk_rightParen);
		    auto oldHasElseCase = hasElseCase; hasElseCase = false;
	    ...
	    ZtAstCase parseCase() {
		    ...
		    if (this.advanceForToken(ZtToken.Type.kw_else)) {
			    if (hasElseCase) error(node.location, "Switch statement cannot have multiple else cases.");
		    ...
		    while(!this.testForToken(ZtToken.Type.kw_case) && !this.testForToken(ZtToken.Type.tk_rightBrace)) {
	    ...
	    ZtAstFor parseFor() {
		    ...
		    this.expectToken(ZtToken.Type.tk_leftParen);
		    if (this.testForToken(ZtToken.Type.kw_def)) node.initializer = this.parseDefParameter();
		    this.expectToken(ZtToken.Type.tk_semicolon);
		    if (!this.testForToken(ZtToken.Type.tk_semicolon)) node.subject = this.parseExpression();
		    this.expectToken(ZtToken.Type.tk_semicolon);
		    if (!this.testForToken(ZtToken.Type.tk_leftParen)) node.step = this.parseExpression();
		    this.expectToken(ZtToken.Type.tk_rightParen);
		    node.members = this.parseBlock!parseStatement();
		    return node;
		}
	    ...
	    ZtAstStatement parseStatement() {
		    nextStatement: switch(buffer.front.type) with(ZtToken.Type) {
	    ...
	    ZtAstDeclaration parseDeclaration() {
		    nextDeclaration: switch(buffer.front.type) with(ZtToken.Type) {
	    ...
	    ZtAstExpression parseExpression(int precedence = int.max) {
		    ...
		    switch(buffer.front.type) with(ZtToken.Type) {
			    ...
			    case kw_new:
				    ...
				    node.type = this.parseReference();
				    this.expectToken(ZtToken.Type.tk_leftParen);
				    if (!this.testForToken(ZtToken.Type.tk_rightParen)) node.arguments = this.parseList!parseExpression();
				    this.expectToken(ZtToken.Type.tk_rightParen);
				    expression = node;
				    break;
			    ...
			    case tk_leftBracket:
				    ...
				    if (!this.testForToken(ZtToken.Type.tk_rightParen)) node.members = this.parseList!parseExpression();
		    ...
		    nextExpression: switch(buffer.front.type) with(ZtToken.Type) {
			    ...
			    case tk_leftBracket:
				    ...
				    node.subject = expression;
				    if (!this.testForToken(ZtToken.Type.tk_rightBracket)) node.arguments = this.parseList!parseExpression();
				    this.expectToken(ZtToken.Type.tk_rightBracket);
				    expression = node;
				    goto nextExpression;
	    ...
	    ZtAstReference parseReference(int precedence = int.max) {
		    ...
		    nextReference: switch(buffer.front.type) with(ZtToken.Type) {
			    case ZtToken.Type.tk_dot:
				    if (Precedence.dispatch > precedence) break;
				    auto node = this.makeNode!ZtAstDispatch(ZtToken.Type.tk_dot);
			    ...
			    case tk_leftBracket:
				    ...
				    auto node = this.makeNode!ZtAstSubscript(ZtToken.Type.tk_leftBracket);
				    node.subject = reference;
				    if (!this.testForToken(ZtToken.Type.tk_rightBracket)) node.arguments = cast(ZtAstExpression[])this.parseList!parseReference();
				    this.expectToken(ZtToken.Type.tk_rightBracket);
				    reference = node;
				    goto nextReference;
	    ...
	    auto parseParamaters(alias func)() {
		    ...
		    if (!this.advanceForToken(ZtToken.Type.tk_rightParen)) {
			    ...
			    if (this.advanceForToken(ZtToken.Type.tk_variadic)) isVariadic = true;
			    this.expectToken(ZtToken.Type.tk_rightParen);
			}
