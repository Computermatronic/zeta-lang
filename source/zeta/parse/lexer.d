    ...
    private {
        ...
        ZtToken parseNumber() {
            ...
            while(!subBuffer.empty && (subBuffer.front.isNumber || subBuffer.front == '.' || subBuffer.front == '_')) {
                if (subBuffer.front == '.' && token.type != ZtToken.Type.ud_float) token.type = ZtToken.Type.ud_float;
                else if (subBuffer.front == '.') break;
                subBuffer.popFront();
            }
            auto end = buffer.length - subBuffer.length;
            token.lexeme = buffer[0..end];
            if (token.type == ZtToken.Type.ud_integer) token.literal = token.lexeme.parseInteger();
